...
test args="" $UNIT_TESTS="1": start-db
  hatch run test {{args}}
...
start-server app="fief.app:app": start-db start-redis compile-translations build-static
  uvicorn --host 0.0.0.0 --port 8000 {{app}}

start-server-watch app="fief.app:app":
  watchexec -r --exts po,py,html,mjs -- just start-server {{app}}

start-worker: start-db start-redis
  dramatiq fief.worker -p 1 -t 1 -f fief.scheduler:schedule
