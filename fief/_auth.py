...
class _AuthManagerBase(typing.Generic[U]):
    """Base class for auth managers."""
...
class AuthManager(_AuthManagerBase[U]):
...
class AsyncAuthManager(_AuthManagerBase[U]):
    """Async class to manage user authentication and methods."""

    def __init__(self, model: type[U], storage: AsyncStorageProtocol[U]) -> None:
        self.model = model
        self.storage = storage

    async def signup(
        self,
        method: AsyncMethodProtocol[EnrollKwargs, AuthenticateKwargs],
        enroll_data: EnrollKwargs,
        **fields: typing.Any,
    ) -> U:
        """
        ...
        existing_user = await self.storage.get_one(
            **{field: fields[field] for field in identifier_fields}
        )
        if existing_user is not None:
            raise UserAlreadyExistsException(
        ...
        user = await self.storage.create(**fields)
    ...
    async def signin(
        ...
        **fields: typing.Any,
        ...
        user = await self.storage.get_one(
            **{field: fields[field] for field in identifier_fields}
        )
        user_id = user.id if user else None
